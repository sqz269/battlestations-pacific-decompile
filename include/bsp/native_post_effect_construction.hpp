...
struct NativePostEffectConstructionContext {
    NativePostEffectOwnerContext& destruction;
    ...
    const char* vertex_format_00d61f0c;
...
struct NativePostEffectConstructionAcquired {
    ...
    NativePostEffectOwner* completed_owner{};
    NativePostEffectReference* completed_reference{};
...
// Caller-owned, immovable storage prepared BEFORE B4E840. All optional owner/
...
class NativePostEffectConstructionBlock final {
public:
    enum class Phase { idle, preparing, prepared, executing, settled };
    NativePostEffectConstructionBlock() noexcept = default;
    ~NativePostEffectConstructionBlock() noexcept;
    NativePostEffectConstructionBlock(const NativePostEffectConstructionBlock&) = delete;
    NativePostEffectConstructionBlock& operator=(const NativePostEffectConstructionBlock&) = delete;
    NativePostEffectConstructionBlock(NativePostEffectConstructionBlock&&) = delete;
    NativePostEffectConstructionBlock& operator=(NativePostEffectConstructionBlock&&) = delete;
    void prepare(NativePostEffectConstructionContext&);
    void cancel_preparation() noexcept;
    Phase phase() const noexcept { return phase_; }
    const NativePostEffectConstructionAcquired& acquired() const noexcept { return acquired_; }
...
private:
    friend void* construct_native_post_effect_00b4e840(void*, std::size_t,
        NativeString&, std::uint32_t, const void*, NativePostEffectConstructionBlock&);
    ...
    static void retire_owner(void*, NativePostEffectReference&) noexcept;
    ...
    std::optional<NativePostEffectOwner> owner_;
    std::optional<NativePostEffectReference> owner_reference_;
...
// Complete B4E840 normal body and its logical twelve-state cleanup schedule.
// Original ECX actual24h/36-byte allocation; name/count/optional input stacked;
...
// all four observed native callers pass null. +04 atomic lifetime begins only at
// original B4E86E count1. No whole-object initialization or extra retain occurs.
...
void* construct_native_post_effect_00b4e840(void* actual_allocation,
    std::size_t allocation_bytes, NativeString& actual_effect_name,
    std::uint32_t primitive_count, const void* optional_size_input,
    NativePostEffectConstructionBlock&);
