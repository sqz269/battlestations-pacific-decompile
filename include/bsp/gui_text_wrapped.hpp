...
struct GuiTextWrappedServices {
    ...
    // Borrowed actual module binding; only native getter type16 calls it.
...
enum class GuiTextWrappedPending {
    ...
    undefined_position_format, // Nonzero+50 type uses uninitialized native scratch.
...
// All initialized getter formats complete; undefined_position_format has no
// native-defined continuation and no resume API. Metrics/ranges are already
// published at that boundary and must not be rerun as a decoder retry.
...
// Retained compatibility leaf; the wrapped builder uses the complete reader
// in native_vertex_position_read.hpp, including initialized packed formats.
