...
class XLiveLibrary final : public XLiveNotificationLibrary,
    public XLiveUpdateInformationHost {
public:
    explicit XLiveLibrary(const std::wstring& absolute_dll_path);
    // Optional caller-selected private dependencies, loaded in order and kept
    // alive until after XLive unloads. Does not change the DLL search path or
    // install files. Each dependency must also have an absolute path.
    XLiveLibrary(const std::wstring& absolute_dll_path,
        const std::vector<std::wstring>& absolute_dependency_paths);
    ...
    std::int32_t x_live_get_update_information(XLiveUpdateInformation&) override;
