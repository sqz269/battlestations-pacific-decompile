...
// Borrow the SAME actual record/string/array/canonical-owner domains.
// Destruction needs no effect-loading operation or construction provider.
// Registry profiles are readable through +10; effect profiles through +0C.
// Numeric native table words are dispatch evidence, never host callables.
struct NativeEffectRegistryDestructionContext {
    NativeEffectRecordStorageContext& actual_records;
    NativeRenderActualOwners& actual_owners;
    const volatile std::uint32_t* actual_base_profile_00d5f04c;
    const volatile std::uint32_t* actual_derived_profile_00d5f074;
    const volatile std::uint32_t* actual_effect_base_profile_00d5e534;
    const volatile std::uint32_t* actual_effect_derived_profile_00d61a00;
};
