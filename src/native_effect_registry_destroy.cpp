...
Word current_effect_size(void* effect, NativeEffectRegistryDestructionContext& c) {
    const auto profile = field<Word>(effect, 0);
    const volatile Word* table = nullptr;
    if (profile == 0x00d5e534u) table = c.actual_effect_base_profile_00d5e534;
    if (profile == 0x00d61a00u) table = c.actual_effect_derived_profile_00d61a00;
...
void current_registry_release(void* registry, void* captured_effect,
    ...
    if (profile == 0x00d5f074u) table = c.actual_derived_profile_00d5f074;
    if (table == nullptr || table[4] != 0x00b32010u)
        throw std::invalid_argument("unsupported current actual effect registry release slot");
    release_native_cached_effect_00b32010(captured_effect, c.actual_owners);
}
struct ArrayUnwind {
    void* const registry;
    NativeEffectRecordStorageContext& context;
    bool armed = true;
...
void clear_native_effect_registry_00b31750(void* registry, NativeEffectRegistryDestructionContext& c) {
    auto& cache = c.actual_records;
    while (field<Word>(registry, 8) != 0) {
        const auto size = current_effect_size(current_last_effect(registry), c);
...
void destroy_native_effect_record_array_00b317c0(void* header, NativeEffectRecordStorageContext& c) {
...
void destroy_native_effect_registry_00b320f0(void* registry, NativeEffectRegistryDestructionContext& c) {
    ...
    ArrayUnwind cleanup{registry, c.actual_records};
    clear_native_effect_registry_00b31750(registry, c);
    cleanup.armed = false;
    destroy_native_effect_record_array_00b317c0(plus(registry, 4), c.actual_records);
}
