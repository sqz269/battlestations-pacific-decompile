...
void unwind_parent(void* owner, I state, void* captured_nested,
    ...
    case 26:
        destroy_native_effect_record_array_00b317c0(at(captured_nested, 4), c.effect_cleanup.actual_records);
        state = 8; break;
    case 27:
...
void __fastcall destroy_native_renderer_00b32920(void* owner, NativeRendererDestructorContext* context) {
    ...
    try {
        ...
        state = 24;
        release_native_tracked_critical_section_0041cc80(static_cast<TrackedCriticalSection**>(at(owner, 0x199c)));
        ...
        state = 8;
        resize_native_effect_records_00b30410(at(captured_nested, 4), 0, c.effect_cleanup.actual_records);
        c.effect_cleanup.actual_records.free_array_00bf6989(pointer(word(captured_nested, 4)));
        state = 7;
