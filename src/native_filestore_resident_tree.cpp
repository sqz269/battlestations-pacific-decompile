...
#error Native FileStore resident trees require MSVC Win32.
#endif
...
[[noreturn]] void throw_invalid_iterator() {
    ...
    // BE67B2 arms state0 after assignment. E015DC -> E015D4 -> CC6D00
    // cleans the temporary; 411700 owns its failed-construction cleanup.
...
void* erase_native_file_store_resident_iterator_00be6760(void* tree, void* output,
    void* iterator_owner, void* iterator_node, NativeStringStorage& strings, NativeAdoptedSubstreamDispatch& dispatch,
    const SingletonLifetimeCallbacks& callbacks) {
    NativeFileStoreNameIterator input{iterator_owner, iterator_node};
    auto* const original = iterator_node;
    if (nil(original)) throw_invalid_iterator();
    advance_native_file_store_resident_iterator_00be4c30(&input, callbacks);
    ...
    if (!transplant) {
        ...
        if (link(current_head) == original) {
            link(current_head) = nil(replacement) ? fixup_parent
                : minimum_native_file_store_resident_node_00be4940(replacement);
        }
        current_head = head(tree);
        if (link(current_head, 8) == original) {
            link(current_head, 8) = nil(replacement) ? fixup_parent
                : maximum_native_file_store_resident_node_00be4920(replacement);
        }
    } else {
        // BE686F..BE68C5 is reachable despite the saved decompiler warning.
        ...
        while (!reached_root && color(replacement) == 1) {
            ...
            if (replacement == sibling) {
                ...
                if (color(sibling) == 0) {
                    ...
                    rotate_native_file_store_resident_left_00be50e0(tree, fixup_parent);
                    sibling = link(fixup_parent, 8);
                }
                if (!nil(sibling)) {
                    if (color(link(sibling)) != 1 || color(link(sibling, 8)) != 1) {
                        if (color(link(sibling, 8)) == 1) {
                            ...
                            color(sibling) = 0;
                            rotate_native_file_store_resident_right_00be4980(tree, sibling);
                            sibling = link(fixup_parent, 8);
                        }
                ...
                if (color(sibling) == 0) {
                    color(sibling) = 1;
                    color(fixup_parent) = 0;
                    rotate_native_file_store_resident_right_00be4980(tree, fixup_parent);
                ...
                if (!nil(sibling)) {
                    if (color(link(sibling, 8)) != 1 || color(link(sibling)) != 1) {
                        if (color(link(sibling)) == 1) {
                            color(link(sibling, 8)) = 1;
                            color(sibling) = 0;
                            rotate_native_file_store_resident_left_00be50e0(tree, sibling);
                            sibling = link(fixup_parent);
                        }
                        color(sibling) = color(fixup_parent);
                        color(fixup_parent) = 1;
    ...
    destroy_native_file_store_payload_00be5d70(at(original, 0x0c), strings, dispatch);
    singleton_lifetime_free(original);
    // Raw BE69DD..BE6A12: current count is unsigned and zero stays zero.
...
void* erase_native_file_store_resident_range_00be7690(void* tree, void* output,
    ...
    if (captured_node == minimum) {
        ...
        if (last_node == captured_head) {
            erase_native_file_store_resident_subtree_00be6720(
                tree, link(head(tree), 4), strings, dispatch);
            auto* current_head = head(tree);
    ...
    for (;;) {
        ...
        advance_native_file_store_resident_iterator_00be4c30(&first, callbacks);
        void* ignored[2];
        erase_native_file_store_resident_iterator_00be6760(tree, ignored,
            captured_owner, captured_node, strings, dispatch, callbacks);
...
void destroy_native_file_store_resident_tree_00be7a30(void* tree, NativeStringStorage& strings, NativeAdoptedSubstreamDispatch& dispatch,
    const SingletonLifetimeCallbacks& callbacks) {
    auto* const captured_head = head(tree);
    auto* const first = link(captured_head);
    void* ignored[2];
    erase_native_file_store_resident_range_00be7690(tree, ignored,
        tree, first, tree, captured_head, strings, dispatch, callbacks);
...
void destroy_native_file_store_resident_tree_00be7bb0(void* tree,
    NativeStringStorage& strings, NativeAdoptedSubstreamDispatch& dispatch,
    const SingletonLifetimeCallbacks& callbacks) {
    // BE7BB0/BE7A30: full 52-byte flows differ only in direct CALL offsets.
    destroy_native_file_store_resident_tree_00be7a30(tree, strings, dispatch, callbacks);
}

void clear_native_file_store_00be72c0(void* provider, NativeStringStorage& strings,
    NativeAdoptedSubstreamDispatch& dispatch, const SingletonLifetimeCallbacks& callbacks) {
    if (word(provider, 0x1c) == 0) return;
    auto* const tree = at(provider, 0x14);
    do {
        auto* const captured_head = head(tree);
        auto* const node = link(captured_head);
        if (node == captured_head) invalid(callbacks);
        if (word(link(node, 0x14), 4) != 1) {
            if (node == head(tree)) invalid(callbacks);
            const auto references = word(link(node, 0x14), 4);
            if (node == head(tree)) invalid(callbacks);
            auto* name = static_cast<const char*>(link(node, 0x10));
            if (!name) name = ""; // Original shared empty byte 0109DB6C.
            // BE7318 is three cdecl arguments, ADD ESP,0C; 4254B0 is RET.
            // Preserve the preceding volatile reads and validation ordering.
            (void)references;
            (void)name;
        }
        NativeFileStoreNameIterator ignored;
        erase_native_file_store_resident_iterator_00be6760(tree, &ignored,
            tree, node, strings, dispatch, callbacks);
    } while (word(provider, 0x1c) != 0);
}
} // namespace bsp
