...
NativeResourceSupportStorage* resource_support_singleton_00b3e730(
    ...
    return resource_support_singleton_00b3e730(
        actual_published_0108fedc, SoundLifetimeAccess(actual_lifetime));
}
...
NativeResourceSupportStorage* resource_support_singleton_00b3e730(
    ...
    {
        CapturedSoundLifetimeSection guard(actual_lifetime);
        if (!actual_published_0108fedc) {
            ...
            auto registration_manager = actual_lifetime.get_manager_00415350();
            registration_manager->register_object(actual_published_0108fedc);
        }
    }
    // Native final global reload occurs AFTER LeaveCriticalSection.
