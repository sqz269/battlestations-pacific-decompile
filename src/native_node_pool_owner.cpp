...
#error Native node pool lifetime requires MSVC Win32.
#endif
...
constexpr std::uint32_t pool_profile = 0x00d62c78;
...
void invoke_trim(void* actual_pool) { trim_native_node_pool_00b6ea60(actual_pool); }

void* actual_canonical_pool;
AllocatorListDomain* actual_canonical_list;
} // namespace

void free_native_node_pool_table_00b6ddf0(void* header) noexcept {
    auto* const current = pointer(header, 0);
    if (current) singleton_lifetime_free(current);
}

void* initialize_native_node_pool_00b6e980(void* pool, AllocatorListDomain& list) {
    auto& element = *::new (pool) AllocatorListElement;
    list.prepend_base_element(element);
    unsigned unwind_state = 0;
    // DFA8C4/DFA8AC: state0 base links, state1 section, state2 table.
    ...
    __try {
        ...
        if (word(pool, 0x30) < 32u) {
            ...
            free_native_node_pool_table_00b6ddf0(at(pool, 0x28));
            pointer(pool, 0x28) = replacement;
        }
    } __finally {
        if (AbnormalTermination()) {
            if (unwind_state >= 2) free_native_node_pool_table_00b6ddf0(at(pool, 0x28));
...
void destroy_native_node_pool_00b6e3d0(void* pool, AllocatorListDomain& list) noexcept {
...
void trim_native_node_pool_00b6ea60(void* pool) noexcept {
    for (std::uint32_t index = 0; index < word(pool, 0x2c); ++index) {
        auto* const slab = pointer(pointer(pool, 0x28), index * 4u);
        if (half(slab, 0x2f40) != 32u) continue;
        ...
        if (index < word(pool, 0x2c)) {
            auto* slot_id = at(pointer(pointer(pool, 0x28), index * 4u), 0x174);
            for (std::uint32_t slot = 0; slot < 32u; ++slot) {
                word(slot_id, 0) = index;
                slot_id = at(slot_id, 0x178);
            }
        }
    ...
    if (has_slabs) {
        ...
        std::uint32_t index = 0;
        do {
            if (half(pointer(cursor, 0), 0x2f40) != 0) {
...
void bind_static_native_node_pool_0108ff58(void* pool, AllocatorListDomain& list) {
    list.bind_virtual0(*static_cast<AllocatorListElement*>(pool),
        {pool_profile, 0x00b6ea60, pool, &invoke_trim});
    actual_canonical_pool = pool;
    actual_canonical_list = &list;
}

int initialize_static_native_node_pool_00cd7d10() {
    initialize_native_node_pool_00b6e980(actual_canonical_pool, *actual_canonical_list);
    return std::atexit(&destroy_static_native_node_pool_00ce0e20);
}

void destroy_static_native_node_pool_00ce0e20() noexcept {
    destroy_native_node_pool_00b6e3d0(actual_canonical_pool, *actual_canonical_list);
}
