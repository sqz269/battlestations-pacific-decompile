...
void append_effect_plane(void* captured_renderer, void* effect,
    ...
    transform_native_effect_direction_0042d0d0_no_normalize(
        direction.data(), basis.data(), static_cast<const float*>(at(scene, 0xf0)));
...
void apply_native_material_pass_00b43410(void* pass, void* entry, void* override_owner,
    ...
    bind_native_renderer_vertex_shader_00b21d10(captured_renderer,
        &c.synchronization, pointer(pass, 0x54));
    bind_native_renderer_pixel_shader_00b21c20(captured_renderer,
        &c.synchronization, pointer(pass, 0x58));
    ...
    for (Word i = 0; i < pair_count; ++i) {
        ...
        if (negative) {
            ...
            // B434DE tests the reloaded selector before reading material+34.
            if (selector < 0) {
                texture = nullptr;
            } else {
                const auto count = static_cast<std::int16_t>(
                    *static_cast<volatile std::uint16_t*>(at(material, 0x34)));
                texture = selector >= count ? nullptr :
    ...
    if (static_cast<std::uint8_t>(indexed)) {
        const Word count = word(section, 0x18); // B435C0 precedes profile capture.
        ...
        const Word count = word(section, 0x18); // B435EE precedes profile capture.
...
void execute_native_material_pass_current08(void* pass, void* entry,
    ...
    require(f.phase == NativeMaterialPassExecutionFrame::Phase::fresh,
        "material pass requires a fresh persistent frame for each invocation");
...
// Genuine base RET4 and borrowed queue-context getter; no substitute providers.
