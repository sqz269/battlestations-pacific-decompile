...
namespace bsp {
namespace {
static_assert(sizeof(void*) == 4, "D3D9 texture2D pool storage targets MSVC Win32.");
static_assert(sizeof(CRITICAL_SECTION) == 24);
static_assert(sizeof(D3D9Texture2DPoolStorage) == 0x38);
static_assert(offsetof(D3D9Texture2DPoolStorage, critical_section_0c) == 0x0c);
static_assert(offsetof(D3D9Texture2DPoolStorage, recursion_24) == 0x24);
static_assert(offsetof(D3D9Texture2DPoolStorage, slabs_28) == 0x28);
static_assert(offsetof(D3D9Texture2DPoolStorage, slab_count_2c) == 0x2c);
static_assert(offsetof(D3D9Texture2DPoolStorage, table_capacity_30) == 0x30);
static_assert(offsetof(D3D9Texture2DPoolStorage, first_free_slab_34) == 0x34);
constexpr std::size_t free_indices = 0xa80;
constexpr std::size_t free_count = 0xac0;
constexpr std::uint32_t no_free_slab = 0xffffffffu;
struct SlabBytes { std::byte bytes[D3D9Texture2DPool::slab_bytes]; };

CRITICAL_SECTION* section(D3D9Texture2DPoolStorage& storage) noexcept {
...
void adjust_recursion(D3D9Texture2DPoolStorage& storage, std::uint32_t delta) noexcept {
    const auto bits = static_cast<std::uint32_t>(storage.recursion_24) + delta;
    std::memcpy(&storage.recursion_24, &bits, sizeof(bits));
}
std::byte* initialize_slab_00b3d1e0(void* raw, std::uint32_t index) noexcept {
    ...
    for (std::uint32_t i = 0; i < 32; ++i) {
        ...
        write<std::uint32_t>(bytes + i * 0x54 + 0x50, index);
    }
    return bytes;
}
...
D3D9Texture2DPool* static_pool_0108db38;
D3D9Texture2DPool& require_static_pool() {
    if (!static_pool_0108db38)
        throw std::logic_error("native 0108db38 D3D9 texture2D pool has no actual storage binding");
    return *static_pool_0108db38;
}
} // namespace

D3D9Texture2DPool::D3D9Texture2DPool(AllocatorListDomain& list,
    D3D9Texture2DPoolStorage& storage) : allocator_list_(list), storage_(storage) {
...
void D3D9Texture2DPool::initialize_00b3ee80() {
    ...
    __try {
        ...
        if (AbnormalTermination()) {
            if (unwind_state >= 2) free_table_unwind_00b3d3b0();
...
void* D3D9Texture2DPool::allocate_raw_slot_00b3ef60() {
    ...
    if (storage_.first_free_slab_34 == no_free_slab) {
        ...
        auto* slab = raw ? initialize_slab_00b3d1e0(raw, storage_.first_free_slab_34) : nullptr;
...
std::uint32_t D3D9Texture2DPool::live_slab_index(const void* slot) noexcept {
    return read<std::uint32_t>(static_cast<const std::byte*>(slot) + slot_slab_index_offset);
}

void D3D9Texture2DPool::return_raw_slot_00b3d8d0(void* slot) {
    EnterCriticalSection(section(storage_));
    adjust_recursion(storage_, 1);
    const auto slab_index = live_slab_index(slot);
    auto* slab = storage_.slabs_28[slab_index];
    // Assembly's signed magic multiply is exact signed division by 0x54.
...
void D3D9Texture2DPool::trim_empty_slabs_00b3e510() {
    ...
    while (index < storage_.slab_count_2c) {
        ...
        // Native continuation 00B3E536 copies even when removing the last item.
...
void D3D9Texture2DPool::invoke_trim(void* context) {
    static_cast<D3D9Texture2DPool*>(context)->trim_empty_slabs_00b3e510();
}

void D3D9Texture2DPool::destroy_critical_section_00402f70() noexcept {
...
void D3D9Texture2DPool::free_table_unwind_00b3d3b0() noexcept {
    if (storage_.slabs_28) singleton_lifetime_free(storage_.slabs_28);
    // Native leaf does not clear table/count/capacity fields.
}

void D3D9Texture2DPool::destroy_00b3e430() {
    storage_.allocator_00.native_vtable_00 = native_vtable;
    for (std::uint32_t i = 0; i < storage_.slab_count_2c; ++i)
        singleton_lifetime_free(storage_.slabs_28[i]);
    free_table_unwind_00b3d3b0();
    destroy_critical_section_00402f70();
    allocator_list_.unlink_base_element_00403970(storage_.allocator_00);
}

void bind_static_d3d9_texture2d_pool_0108db38(D3D9Texture2DPool& pool) {
    if (static_pool_0108db38 && static_pool_0108db38 != &pool)
        throw std::logic_error("native 0108db38 already has a different D3D9-texture2D-pool binding");
    static_pool_0108db38 = &pool;
}

int initialize_static_d3d9_texture2d_pool_00cd7b60(D3D9Texture2DPoolAtexit register_atexit) {
    if (!register_atexit) throw std::invalid_argument("CRT atexit registration is required");
    require_static_pool().initialize_00b3ee80();
    // Native returns CRT registration's result; failure does not undo the pool.
    return register_atexit(&destroy_static_d3d9_texture2d_pool_00ce0ca0);
}

void destroy_static_d3d9_texture2d_pool_00ce0ca0() {
    require_static_pool().destroy_00b3e430();
}
void* allocate_d3d9_texture2d_slot_00b3f2b0() {
    return require_static_pool().allocate_raw_slot_00b3ef60();
}
void return_d3d9_texture2d_slot_00b3dcd0(void* slot) {
    require_static_pool().return_raw_slot_00b3d8d0(slot);
}
} // namespace bsp
