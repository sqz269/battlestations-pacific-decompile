...
void build_native_material_constants_00b42350(void* pass, void* entry,
    ...
    void* const* const parameter_table = get_native_material_parameter_table_00b17390(
        static_cast<const NativeMaterialStorage*>(ptr(ptr(entry, 4), 0x20)));
    const Word type = word(ptr(entry, 0x10), 0x198);
    void* parameter_slot = at(parameter_table);
            ...
            if (byte(parameter, 0x10) != 0) {
                ...
                while (constant != constant_end) {
                    ...
                    if (static_cast<Word>(native_shader_constant_register_00b5b870(constant)) == vertex_register) {
                        ...
                        rows = get_native_shader_constant_register_count_00b5b880(constant);
                    }
    ...
    if (byte(ptr(pass, 0x70), 0x30) != 0xff) {
        ...
        const Signed count = get_native_model_bone_count_00b8ff00(object);
        ...
        for (Signed index = 0; index < count; ++index) {
            ...
            void* const node = get_native_model_bone_node_00b90620(object, 0u, static_cast<Word>(index));
    ...
    if (skin_register != 0xff) {
        ...
        if (animator) {
            ...
            const Word required_type = get_native_optimized_animator_type_id_00b75e50(*c.actual_types_010900fc);
    ...
    if (byte(shader, 0x20) != 0xff) {
        ...
        while (ordinal < word(section, 0x4c)) {
            ...
            Word count = get_native_vertex_declaration_element_count_00b47900(declaration);
            ...
            if (static_cast<Signed>(count) > 0) {
                ...
                do {
                    if (decoded) {
                        ...
                        const void* const decode = get_native_stream_decode_record_00b61e10(logical, 0u, index);
    ...
    if (word(pass, 0x78) != 0xffffffffu) {
        ...
        void* const shadow = get_native_light_shadow_map_owner_00b7aab0(ptr(first, 8));
        if (shadow) {
