...
GeneratedModelPointLightLinks& DirectionalLightReference::light_element(void* context,
    std::uint32_t index) noexcept {
    const auto& array = static_cast<DirectionalLightReference*>(context)->owner_.node.storage.point_lights_164;
    auto* light = array.begin[index]; // current begin, after the shared live-count query
