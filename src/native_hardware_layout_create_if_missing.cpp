...
// Native frame+48h..100h: 14 usage counters, 16 packed eight-byte slots, live
...
void __fastcall create_native_hardware_layout_if_missing_00b60a10(
    void* owner, NativeHardwareLayoutConstructContext* actual_context) {
    // This load is the entire hit path: do not even form a context member here.
    if (word(owner, 0x40) != 0) return;
    const auto count = word(owner, 0x38);
    std::uint32_t streams[4];
    std::uint32_t captured_count = 0;
    // Native JBE/JC are unsigned; >4 (including negative bit patterns) is
    // outside the original four-pointer scratch domain, not a signed skip.
    for (std::uint32_t i = 0; i < count; ++i) {
        put(streams, captured_count * 4u, word(owner, 8u + i * 12u));
        ++captured_count;
    }
    auto& context = *actual_context;
    Scratch scratch;
    for (std::uint32_t i = 0; i != 14; ++i) put(scratch.usage_counts, i * 4u, 0);
    const auto initial_count = signed_word(captured_count);
    ...
    if (initial_count > 0) {
        do {
            ...
            ++stream_index;
        } while (signed_word(stream_index) < signed_word(captured_count));
    }
