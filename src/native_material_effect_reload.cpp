...
void release_derived(NativeMaterialEffectStorage& effect,
    ...
    // B45EE0 installs a retained callable binding on this SAME descriptor.
    // Raw D61A44 is an original numeric table, never a host function pointer.
    ...
    if (numeric)
        throw std::logic_error("raw descriptor reload requires the missing numeric terminal provider");
...
void reload_native_material_effect_00b469a0(NativeMaterialEffectStorage& effect,
    ...
    if (&context.programs.load_variants_0108d6f0 !=
        &context.shader_modes.load_variants_0108d6f0)
        throw std::invalid_argument("reload requires the programs' canonical shader mode cells");
    if (!context.shader_modes.source_mode_0108d6f1) {
