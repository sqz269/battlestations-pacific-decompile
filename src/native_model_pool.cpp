...
namespace bsp {
namespace {
static_assert(sizeof(void*) == 4, "Native model pool storage targets MSVC Win32.");
static_assert(sizeof(CRITICAL_SECTION) == 24);
static_assert(sizeof(NativeModelPoolStorage) == 0x38);
static_assert(offsetof(NativeModelPoolStorage, critical_section_0c) == 0x0c);
static_assert(offsetof(NativeModelPoolStorage, recursion_24) == 0x24);
static_assert(offsetof(NativeModelPoolStorage, slabs_28) == 0x28);
static_assert(offsetof(NativeModelPoolStorage, slab_count_2c) == 0x2c);
static_assert(offsetof(NativeModelPoolStorage, table_capacity_30) == 0x30);
static_assert(offsetof(NativeModelPoolStorage, first_free_slab_34) == 0x34);
constexpr std::size_t free_indices = 0x3100;
constexpr std::size_t free_count = 0x3140;
constexpr std::uint32_t no_free_slab = 0xffffffffu;
struct SlabBytes { std::byte bytes[NativeModelPool::slab_bytes]; };

CRITICAL_SECTION* section(NativeModelPoolStorage& storage) noexcept {
...
void adjust_recursion(NativeModelPoolStorage& storage, std::uint32_t delta) noexcept {
    const auto bits = static_cast<std::uint32_t>(storage.recursion_24) + delta;
    std::memcpy(&storage.recursion_24, &bits, sizeof(bits));
}
std::byte* initialize_slab_00b74450(void* raw, std::uint32_t index) noexcept {
    ...
    for (std::uint32_t i = 0; i < 32; ++i) {
        ...
        write<std::uint32_t>(bytes + i * 0x188 + 0x184, index);
    }
    return bytes;
}
...
NativeModelPool* static_pool_01090054;
NativeModelPool& require_static_pool() {
    if (!static_pool_01090054)
        throw std::logic_error("native 01090054 Native model pool has no actual storage binding");
    return *static_pool_01090054;
}
} // namespace

NativeModelPool::NativeModelPool(AllocatorListDomain& list,
    NativeModelPoolStorage& storage) : allocator_list_(list), storage_(storage) {
...
void NativeModelPool::initialize_00b74b80() {
    ...
    __try {
        ...
        if (AbnormalTermination()) {
            if (unwind_state >= 2) free_table_unwind_00b74530();
...
void* NativeModelPool::allocate_raw_slot_00b74d00() {
    ...
    if (storage_.first_free_slab_34 == no_free_slab) {
        ...
        auto* slab = raw ? initialize_slab_00b74450(raw, storage_.first_free_slab_34) : nullptr;
...
std::uint32_t NativeModelPool::live_slab_index(const void* slot) noexcept {
    return read<std::uint32_t>(static_cast<const std::byte*>(slot) + slot_slab_index_offset);
}

void NativeModelPool::return_raw_slot_00b74750(void* slot) {
    EnterCriticalSection(section(storage_));
    adjust_recursion(storage_, 1);
    const auto slab_index = live_slab_index(slot);
    auto* slab = storage_.slabs_28[slab_index];
    // Assembly's signed magic multiply is exact signed division by 0x188.
...
void NativeModelPool::trim_empty_slabs_00b74c60() {
    ...
    while (index < storage_.slab_count_2c) {
        ...
        // Native continuation 00B74C86 copies even when removing the last item.
...
void NativeModelPool::invoke_trim(void* context) {
    static_cast<NativeModelPool*>(context)->trim_empty_slabs_00b74c60();
}

void NativeModelPool::destroy_critical_section_00402f70() noexcept {
...
void NativeModelPool::free_table_unwind_00b74530() noexcept {
    if (storage_.slabs_28) singleton_lifetime_free(storage_.slabs_28);
    // Native leaf does not clear table/count/capacity fields.
}

void NativeModelPool::destroy_00b74690() {
    storage_.allocator_00.native_vtable_00 = native_vtable;
    for (std::uint32_t i = 0; i < storage_.slab_count_2c; ++i)
        singleton_lifetime_free(storage_.slabs_28[i]);
    free_table_unwind_00b74530();
    destroy_critical_section_00402f70();
    allocator_list_.unlink_base_element_00403970(storage_.allocator_00);
}

void bind_static_native_model_pool_01090054(NativeModelPool& pool) {
    if (static_pool_01090054 && static_pool_01090054 != &pool)
        throw std::logic_error("native 01090054 already has a different native-model-pool binding");
    static_pool_01090054 = &pool;
}

int initialize_static_native_model_pool_00cd7f00(NativeModelPoolAtexit register_atexit) {
    if (!register_atexit) throw std::invalid_argument("CRT atexit registration is required");
    require_static_pool().initialize_00b74b80();
    // Native returns CRT registration's result; failure does not undo the pool.
    return register_atexit(&destroy_static_native_model_pool_00ce0e50);
}

void destroy_static_native_model_pool_00ce0e50() {
    require_static_pool().destroy_00b74690();
}
void* allocate_native_model_slot_00b74eb0() {
    return require_static_pool().allocate_raw_slot_00b74d00();
}
void return_native_model_slot_00b748c0(void* slot) {
    require_static_pool().return_raw_slot_00b74750(slot);
}
} // namespace bsp
