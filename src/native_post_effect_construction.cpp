...
void require_renderer(void* actual, const NativePostEffectConstructionContext& context,
...
void release_registered(void* actual, NativePostEffectDecrement decrement,
    NativePostEffectConstructionContext& context, Word profile,
...
NativeModelOwner& current_model(void* actual, NativePostEffectConstructionContext& context) {
...
NativeCameraOwner& current_camera(void* actual, NativePostEffectConstructionContext& context) {
    auto* binding = context.destruction.nodes.attachments.find_actual_node(bits(actual));
...
Word surface_dimension(const void* actual, NativePostEffectConstructionContext& context, bool height) {
...
NativePostEffectConstructionBlock::~NativePostEffectConstructionBlock() noexcept {
    if (phase_ != Phase::idle) std::terminate();
}
void NativePostEffectConstructionBlock::validate() const {
    auto& c = *context_;
    ...
    require(c.renderer_profile_00d5f0a8 && c.surface_profile_00d619a0 && c.viewport_profile_00d5e5f8 &&
        c.vertex_format_00d61f0c && c.model_name_00d61ee4 && c.camera_prefix_00d61ed0 &&
    ...
    require(reinterpret_cast<const volatile void*>(&c.actual_renderer_00f8d394) ==
        ...
        c.streams.actual_logical_profile_00d61d6c == c.destruction.actual_stream_profile_00d61d6c &&
...
void NativePostEffectConstructionBlock::prepare(NativePostEffectConstructionContext& context) {
...
void NativePostEffectConstructionBlock::cancel_preparation() noexcept {
...
void NativePostEffectConstructionBlock::settle() noexcept {
    camera_scene_.cancel(); camera_lifetime_.cancel();
    for (auto& admission : viewport_admissions_) admission.cancel();
    phase_ = Phase::settled;
}
void NativePostEffectConstructionBlock::bind(Slot slot, void* actual, RenderCommandReference& reference) {
    keys_[slot] = actual;
...
void NativePostEffectConstructionBlock::retired(Slot slot, RenderCommandReference& reference) noexcept {
...
#define BSP_BO_RETIRE(name, type, slot) \
void NativePostEffectConstructionBlock::name(void* block, type& reference) noexcept { \
    static_cast<NativePostEffectConstructionBlock*>(block)->retired(slot, reference); \
}
BSP_BO_RETIRE(retire_mesh, NativeMeshReference, mesh)
BSP_BO_RETIRE(retire_section, NativeMeshSectionReference, section)
BSP_BO_RETIRE(retire_material, NativeMaterialReference, material)
BSP_BO_RETIRE(retire_stream, NativeLogicalVertexReference, stream)
BSP_BO_RETIRE(retire_model, NativeModelReference, model)
BSP_BO_RETIRE(retire_camera, NativeCameraReference, camera)
BSP_BO_RETIRE(retire_owner, NativePostEffectReference, owner)
#undef BSP_BO_RETIRE

void NativePostEffectConstructionBlock::reset_after_host_quiescence() noexcept {
    ...
    if ((model_owner_ && model_owner_->phase != NativeModelOwner::Phase::dead) ||
        ...
        (owner_ && owner_->phase() != NativePostEffectOwner::Phase::dead)) std::terminate();
...
void NativePostEffectConstructionBlock::string_cleanup(NativeString& header) {
...
void NativePostEffectConstructionBlock::unwind(int& state, void*& frame_raw, void*& mesh_raw,
    void*& late_raw, Word& mask) {
    // DF8758/DF877C: transition before action; a second C++ cleanup failure
...
void* construct_native_post_effect_00b4e840(void* actual, std::size_t allocation_bytes,
    NativeString& effect_name, Word primitive_count, const void* optional_size_input,
    NativePostEffectConstructionBlock& block) {
    require(block.phase_ == NativePostEffectConstructionBlock::Phase::prepared && actual &&
        !(bits(actual) & 3u) && allocation_bytes >= 0x24u,
        "post-effect constructor requires prepared storage and an aligned actual 24h allocation");
    block.validate();
    auto& c = *block.context_;
    auto& acquired = block.acquired_;
    acquired.actual_owner = actual;
    block.phase_ = NativePostEffectConstructionBlock::Phase::executing;
    int state = -1;
    Word mask = 0;
    ...
    try {
        put(actual, 0, 0x00ceb130u); // B4E863
        new (pointer(bits(actual) + 4u)) std::atomic<std::int32_t>(1); // B4E86E, sole initial count write
        put(actual, 0, 0x00d61ec8u); // B4E87F
        put(actual, 0x0c, 0); put(actual, 0x10, 0); put(actual, 0x14, 0);
        put(actual, 0x20, primitive_count);
        set_state(0);
        frame_raw = allocate(0x40); // B4E891, raw CRT allocation identity
        set_state(1);
        auto* frame = frame_raw ? construct_native_frame_target_owner_00b1fbb0(frame_raw) : nullptr;
        set_state(0);
        acquired.frame_created = frame;
        put(actual, 0x08, bits(frame)); // B4E8BD

        resize_native_string_header_0041dd40(&block.declaration_name_, c.raw_strings, 15, true);
        if (void* data = pointer(word(&block.declaration_name_, 4)))
            std::memcpy(data, c.vertex_format_00d61f0c, word(&block.declaration_name_) + 1u);
        void* renderer = c.actual_renderer_00f8d394; // B4E8EB, fresh current renderer
        ...
        set_state(0); // B4E90A, before normal string cleanup
        ...
        renderer = c.actual_renderer_00f8d394; // B4E927: not the earlier captured receiver
        require_renderer(renderer, c, 0x5c, 0x00b287c0u);
        const Word vertex_count = primitive_count * 3u;
        void* stream = create_native_registered_vertex_stream_00b287c0(renderer, vertex_count,
            0x1000u, declaration, c.streams, &acquired.stream_created);
        acquired.stream_created = stream;
        put(actual, 0x18, bits(stream)); // B4E947, before declaration decrement
        if (stream) {
            block.stream_reference_.emplace(stream, c.streams,
                NativeLogicalVertexCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_stream});
            acquired.stream_reference = &*block.stream_reference_;
            block.bind(NativePostEffectConstructionBlock::stream, stream, *block.stream_reference_);
        }
        ...
        if (mesh) {
            block.mesh_reference_.emplace(*mesh, c.meshes,
                NativeMeshCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_mesh});
            acquired.mesh_reference = &*block.mesh_reference_;
            block.bind(NativePostEffectConstructionBlock::mesh, mesh, *block.mesh_reference_);
        }
        void* current_stream = pointer(word(actual, 0x18)); // B4E988
        require(mesh != nullptr, "native post-effect mesh dereference requires successful allocation");
        set_native_mesh_vertex_stream_00b73bb0(*mesh, c.destruction.actual_owners, 0, current_stream);
        ...
        NativeMaterialStorage* material = create_native_material_from_effect_cache_00535320(effect_name,
            ...
            c.material_effects, *block.material_factory_); // B4E99C
        acquired.material_created = material;
        put(actual, 0x14, bits(material)); // B4E9A5
        ...
        block.material_reference_.emplace(*material, c.materials, c.destruction.actual_material_profile_00d5e520,
            NativeMaterialCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_material});
        acquired.material_reference = &*block.material_reference_;
        block.bind(NativePostEffectConstructionBlock::material, material, *block.material_reference_);
        ...
        block.section_reference_.emplace(*section, c.sections,
            NativeMeshSectionCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_section});
        acquired.section_reference = &*block.section_reference_;
        block.bind(NativePostEffectConstructionBlock::section, section, *block.section_reference_);
        put(section, 0x08, 4); put(section, 0x0c, 0); put(section, 0x10, vertex_count);
        put(section, 0x14, 0); put(section, 0x18, primitive_count);
        ...
        put(actual, 0x10, bits(model)); // B4EA33
        set_state(0);
        if (model) {
            block.model_reference_.emplace(*block.model_owner_,
                NativeModelCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_model});
            acquired.model_reference = &*block.model_reference_;
            block.bind(NativePostEffectConstructionBlock::model, model, *block.model_reference_);
        }
        if (mask & 1u) { mask &= ~1u; block.string_cleanup(block.model_name_); }
        float scalar_17c, scalar_178;
        const volatile Word* unchanged = &c.models.constants.unchanged_00d7a260;
        void* current_model_actual;
        // B4EA60..74: one FLD, current receiver read, FST then FSTP.
        __asm {
            mov eax, unchanged
            fld dword ptr [eax]
            mov eax, actual
        ...
        put(actual, 0x0c, bits(camera)); // B4EAC5
        set_state(0);
        if (camera) {
            block.camera_reference_.emplace(*block.camera_owner_,
                NativeCameraCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_camera},
                std::move(block.camera_lifetime_));
            acquired.camera_reference = &*block.camera_reference_;
            block.bind(NativePostEffectConstructionBlock::camera, camera, *block.camera_reference_);
        }
        ...
        if (viewport) {
            ...
            auto decrement = c.destruction.actual_decrement_00ce2220; // B4EB5C, independent epoch
        ...
        if (late_raw) {
            ...
            // B4EB8A..9F: constants and current model/camera reads retain native order.
            __asm {
                fld1
                mov eax, actual
        ...
        put(actual, 0x1c, bits(draw)); // B4EBAB
        set_state(0);
        acquired.draw_record_created = draw;
        auto decrement = c.destruction.actual_decrement_00ce2220; // B4EBB5: ONE epoch for both releases
        ...
        acquired.native_completed = true; // HOST-only: original B4E840 has completed.
        set_state(-1);
        ...
        block.owner_reference_.emplace(*block.owner_, c.declaration_companions.registration(),
            NativePostEffectCompanionDisposal{&block, &NativePostEffectConstructionBlock::retire_owner});
        acquired.completed_reference = &*block.owner_reference_;
        block.keys_[NativePostEffectConstructionBlock::owner] = actual;
        block.references_[NativePostEffectConstructionBlock::owner] = &*block.owner_reference_;
