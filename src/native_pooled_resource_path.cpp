...
void copy_current_nonempty_source(void* destination, const void* source) {
    if (read_header<std::uint32_t>(source, 0) != 0) {
        ...
        if (count != 0) std::memcpy(data, source_data, count);
    }
}
...
void normalize_native_resource_path_header_00bee690(void* actual_header,
    ...
    try {
        if (actual_header != returned_result) {
            ...
            copy_current_nonempty_source(actual_header, returned_result);
        }
    } catch (...) {
...
void* copy_construct_native_resource_path_header_00bee780(void* actual_output_header,
    ...
    if (!identical) {
        ...
        copy_current_nonempty_source(actual_output_header, actual_source_header);
    }
    // Native flag bit 0 is armed only here (00BEE7DD), after initial copy.
    bool output_constructed = true;
    try {
        normalize_native_resource_path_header_00bee690(actual_output_header, storage);
    } catch (...) {
