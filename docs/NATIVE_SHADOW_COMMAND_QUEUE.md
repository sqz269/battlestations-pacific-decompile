...
C3 for each one-byte RET; both complete four-byte generated getters exactly match the native bytes without relocations.
...
and every numeric native CALL with its containing function. Original placeholder prototypes were not treated as register-ABI evidence. The primary subsequently saved all four reviewed native prototypes/names/comments, retained old values and refreshed exports.
The canonical live call audit checks nine direct rows; all nine pass.

The coupled source `4d0a0cbd5a27bceec34d39576228dc2b309397e6` passes the Win32 build and both existing math CTests with all 2,648 actual inputs and clean exact HEAD unchanged. No new tests, runtime fixtures or game execution were added. The first parallel attempt stopped on a CMake timestamp access error; an otherwise identical serial rerun passed with both entire packet objects and all four libraries unchanged. Both attempts are retained.

Primary and independent review covers all 82 COFF sections, 28 code sections, 1,104 generated bytes, 415 instructions and 138 relocations. The queue 100-byte body and job 52-byte body preserve the reviewed original public-slot/current-read/callback order and all normal stack exits through the added EDX access context. Both getters are exact four-byte MOV/C3 bodies. The 3-byte current-word helper, 19-byte reserve and 11-byte upload adapters, 138-byte constructor and 111-byte dispatcher bind the documented actual providers. Source-specific code totals 442 bytes; 662 compiler/library-support bytes receive no native reconstruction credit. The complete object is an exact current library member.

The independent review originally examined the failed-build object. Primary verified its whole-object identity, unchanged source/provider inputs and current library provenance after the passing rerun. The final report retains both evidence boundaries. The batch admits 132 original normal-body bytes here and 175 vector bytes, not generated helper/compiler bytes. Parent A8EA20/A8BD20, full queue/job lifetime, native fault/unwind ABI, renderer execution and gameplay validation remain incomplete.
