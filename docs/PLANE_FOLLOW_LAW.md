...
**Not yet bound, and the honest blocker for switching the host off placement:** the steer point
itself. `009BFEE0`'s fly-to arm `009C0026`-`009C16D1` (~1500 instructions, six distinct
`state+44h` store sites, so at least that many regimes) is still unread, and it is the body
that decides *where* a member is told to head. The commanding law above consumes that point; it
cannot manufacture it. Substituting the station point for the steer point collapses `t` to 1
and the altitude blend to `stationY`, which is a coherent but strictly weaker law than the
image's.
...
| `009BFEE0` fly-to arm | `009C0026`-`009C16D1` | **OPEN** — the blocker; six `+44h` store sites |
...
Auxiliary state fields `009BFEE0`'s fly-to arm also writes, consumer not yet identified:
`+50h`, `+54h`, `+58h`, `+5Ch`, `+60h`, `+64h`, `+68h`.
