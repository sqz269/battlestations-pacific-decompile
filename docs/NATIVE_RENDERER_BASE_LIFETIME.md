...
## Main integration (R31)

The nine base-lifetime source bodies are reused from `b521b0a59` on the orch5 branch. The current integration preserves main's existing raw singleton and tracked-section providers. It does not adopt the derived renderer constructor/destructor or replace singleton deletion tables. Fresh validation is recorded in `reports/native_renderer_base_lifetime_main_r31.json`; the earlier report retains its historical branch scope.
