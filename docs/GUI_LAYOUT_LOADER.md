...
## Corrections from docs/GUI_LAYER_MANAGER.md

`cGuiLayer` is the page class itself: widget-factory type id 1, 0x124 bytes, vtable `00d5be38`, the element type of the page vector at manager+14h, so the `GuiScreen` class here and the render-order doc's `cGuiLayer` are one class. `Priority` at +FCh orders the update and hit-test list, `RenderOrder` at +11Ch keys the draw map, the load flag at layer+120h gates camera-store scene reuse, and the layer name is a NativeString object starting at +100h, not a pointer. The property describer emits `Priority` only, so a describe/read round trip drops `RenderOrder`; hit resolution picks the smallest `size.x * scale.x` containing the pointer, never reading Z.
