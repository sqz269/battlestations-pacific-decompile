...
- docs/FRONTEND_MANAGERS.md: the three front-end manager singletons, the attract screen and the in-mission interface manager share one 40h-byte base (constructor `00684e10`, vtable `00cf76d0`: two 1Ch-byte interface-request records at +04h/+20h, an active byte at +3Ch, membership in the manager map at `00e19898`). `*(00e198ac)+4h` is the applied interface id (index into the 45-entry name table at `00e08cd8`), not a count. Requests 16h/06h/09h only deactivate through `00683aa0`.
- docs/MISSION_STATE_ENTRY.md: `004db920` `BSP_Game_UpdateDeviceWaitScreen` does not wait for fewer than two active players: it splits on the local view mode and `[game+1EF0h+29Ch]`; one arm is a rising-edge press-any-button over the device table (previous sample at +DDh, suppressed by the menu screen and the GUI root), the other, in view mode 1 only, waits until no in-use local player slot is left unbound, a controller-assignment wait. Both converge on `004da6c0` `BSP_Game_EnterMissionState`, which zeroes the scaled frame delta, releases the `game+30h` dynamics list, rearms the five one-shots at `game+1EE1h..1EE5h`, promotes the selected local slot, writes state 0Dh and clears both cinematic flags plus `game+608h` so the simulation gate opens.
- docs/UNIT_INSTANCE_UPDATE.md: step 5 (`004e4a80..004e4b02`) is an inlined copy of `004c0890` `BSP_Game_SetControlledUnit`: `DAT_00e188d8` is the player-controlled unit, not a device, and the vtable+5Ch probes are `IsKindOf` calls.
