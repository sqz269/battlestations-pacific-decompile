...
## Correction from docs/UNIT_CONTROLLER_UPDATE.md

`0092be80`, the step-6 call on the object at unit+1018h, is a single `RET 4` reached by direct calls, so the unit's per-frame call into its controller does nothing. That object is a 0x390-byte hydrodynamic force model (base constructor `00939cb0`, four virtual creators) with a one-slot force vtable and an embedded contact listener at +20h; its force callback `009329c0` is invoked by the in-house rigid-body library (no middleware strings or RTTI exist), samples the ocean per buoyancy element of the descriptor, drains the unit's external force list at +10D4h and flushes AddForce/AddTorque, and its contact callback `009377e0` produces the collided latch at unit+1010h. Motion is integrated by the library; the controller has no throttle, rudder or target speed, and commanded force arrives only through the +10D4h list, so player and AI units are indistinguishable at this level.
