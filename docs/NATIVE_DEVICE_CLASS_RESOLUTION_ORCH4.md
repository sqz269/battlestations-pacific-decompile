...
| `008798F0` uninitialized section fill | [Native section fill](NATIVE_DAMAGEABLE_SECTION_FILL_ORCH4.md) | Complete 157-byte body and catch audit; original/source normal-path comparison with genuine copy provider, zero/null/aliased cases; three call rows, independent review, strict Win32 and combined 3 CTests |
...
`local/worker_evidence/damageable-section-e5/`; the fill comparison is under
`local/worker_evidence/section-fill-e5/`. The corresponding worker worktrees
can be removed without losing that local validation evidence.
