...
object. Primary and independent source/generated reviews now have no remaining
finding at that exact tested object. The independent review covered 29 complete
code sections, 24 data sections and 146 instruction anchors; the primary review
checked the complete source/native bodies and targeted generated scheduling.

The generated name cleanup captures length before the current pool getter, then
may increment that saved value after the getter. It does not reread the header;
this preserves captured-length-plus-one behavior without claiming original
EFLAGS or instruction placement. Compiler FH3 metadata is distinct from the
explicit native 19/3/1 cleanup projections.

Ghidra names, native __thiscall prototypes and evidence comments for all three
functions are saved in the existing bsp project. Live readback confirms one
implicit this parameter and unchanged bounds; fresh code/assembly exports are
retained. Export metadata's name field comes from the earlier snapshot inventory;
the live prototype/comment evidence records the current reviewed names.

The sealed artifact is `local/dz-validated-attempt-c6ea046e.zip`, SHA256
`a12abdbf02d77403b4732077b2da3ef639fc658d59ec604754680b37d80e8935`, with 2,695 payloads plus its manifest, all reread and hash checked.
It contains the actual tested worker inputs, four libraries, object, two math
test executables, build/test configuration and logs, source/native/generated
reviews and refreshed Ghidra exports. All 2,631 tracked Git inputs match the root;
2,622 raw files match and nine differ only by CRLF/LF. Those root variants are
archived separately. The additional ignored verified seed header matches too.
No second root build was run. Existing math tests and generated inspection do
not establish constructor runtime behavior, original ABI/private FH3/SEH,
native-body generated byte identity or game validation.
