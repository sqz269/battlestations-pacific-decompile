...
DFAAD4 uses maxState2/mapDFAAC4: {-1,CC1B40},{0,CC1B48}. DFAB08 uses maxState2/mapDFAAF8: {-1,CC1B60},{0,CC1B68}. Both have19930522 magic. Constructor state0 follows925490 and precedes name initialization; state1 follows the source/self comparison. Destructor starts state1, consumes name before its getter/return, then consumes weak before925540. The C++ try/catch schedules use these boundaries; native private stack, FH3/SEH transport and faults are outside the contract. The worker made no Ghidra mutations. Primary integration subsequently defined both10B handlers under the write lock; the original undefined-function observations remain in the report. All15 direct/tail rows now receive instruction verification; the2 indirect rows remain separate.
