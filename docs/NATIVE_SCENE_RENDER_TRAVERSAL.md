...
The final coupled build confirms that both helpers contain only integer instructions and no calls or FP operations.
...
The primary defined the complete B6FB80 camera body (36 bytes, 11 instructions), preserved the unavailable original comment query as unavailable evidence, and saved all three reviewed names/native prototypes/comments. All six direct report call rows now pass. The three indirect-slot rows remain explicitly unverified by that direct-call tool; native instruction/table evidence is retained.

Final tested source `ac3784a5f737fe6b5b384a06901ea033e39d5a5d` passes the Win32 build and both existing math CTests with all 2,643 tracked inputs plus the actual verified seed unchanged and exact clean HEAD before/after. There are no new tests or runtime fixtures. Full current scene and renderer objects are exact members of the final library and received independent generated review. Scene 125, node 143 and camera 44 generated bytes retain the original FP spill/current-read schedules within the documented source domain. Integer translation 53 and selector 65 bytes, captured dispatch 46, fallback entry 25/adapter 68, constructor 114 and missing-profile 34 bytes were fully reviewed. Compiler exception/RTTI support has zero native credit.

Of the existing renderer's 28 code sections, 27 (5,865 bytes) retain their prior tested bytes and normalized relocations. The changed dispatch bridge is 95 bytes, previously 113. Its profile callback remains after the caller's original FP spills. Its current slot20 load now precedes its local SSE argument copies; the previous emitted bridge loaded slot20 after its local visibility MOVSS load. This is a documented C++ bridge boundary, not prior emitted instruction or arbitrary stack-alias identity.

The first combined build at `cd637cb1e8e841b790d33abaceb2d5a061ae2c12` failed on reserved MSVC `bound` labels before tests. The primary changed only four label/reference tokens. The en2 build at `07dd326abfdf78a6b80804036af2ce2e180841f6` passed both tests, but generated review found two EN helpers encoded as three-byte RET0 instead of native one-byte RET. Exactly those two helpers now emit C3 explicitly. The final en3 build passes that comparison, and both complete EL objects are byte-identical to en2. Every earlier attempt, including its actual inputs and full build tree, remains archived.

These checks establish 225 bytes across the three original normal bodies, within the finite borrowed-table source contract. They do not establish runtime rendering, gameplay, original exception/FP-fault ABI, complete queue/job closure, or a runnable renderer. The final report records the complete validation archive and exact source/build/object hashes.
