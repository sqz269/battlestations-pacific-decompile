...
- Phase 2 (docs/APP_INIT_VFS_SINGLETONS.md): `00beda60` does not store `DAT_0109ceec`; the store is two calls deeper, in `00bda6f0` through `00be1dc0`, under the singleton-lifetime lock together with the lifetime registration, and `00bda790` zeroes it. The third mount takes `filestore` as its *system* path and `.` as its virtual path, the reverse of the two physical mounts, and pushes ownership byte 0. `manager+8Ch` receives `00735b30`, a single `C3` plus INT3 padding with no Ghidra function, so both handlers are verified no-ops. A fourth provider factory, MPAK (`00736b60`, singleton `DAT_010904d4`, accepts only system paths ending in `.mpak`), is registered at `0073d955` after both package scans; `00736c30` is a 1Ch-byte reference-counted registry in `DAT_010904d8` published at `manager+88h`.
- Phase 10 (docs/APP_INIT_FONTS_GUI.md): the GUI manager is created inside `0073bae0` through `004c12b0`, before the `After InitGui` checkpoint, not after it. `0073c960` is the localisation table manager published in `DAT_00f8bc4c`, not a GUI structure; its 0x4000-byte table is the hash bucket array of 0x1000 four-byte heads that `0073c240`, `00a9fc30` and `00a9ec70` index with the `0xfff` mask. `004c14c0` is a diagnostic sink singleton (lazy construction plus lifetime registration). `007371d0` takes no arguments; the three string pushes at `0073bb7e` belong to `00ac3910` (`RET 0Ch`).
- Phases 4, 7 and 9 (docs/APP_INIT_RENDERER.md): the unresolved indirect call in step 53 resolves to a bare `RET` on the object step 51 creates; the shader preload `0073bf80` runs on the application object, not the renderer, and loads each script entry through the renderer effect registry; renderer virtual `+104h` (`00b1ff50`) returns the capability record at renderer+1B18h and `+F8h` (`00b21ec0`) wraps `CheckDeviceFormat`; the shadow map is square, 4096 or 2048 by `HiResShadow`.
- Phase 9 (docs/APP_INIT_WORLD_EFFECTS.md): `interface/textures/common.ats` does not exist in the installation; the predicate at `00aef3c0` is a split-atlas prefix rule, so one request loads every `<stem>_*.ats` sibling (four `common_*` parts are installed). The atlas loader's dedupe guard compares registered texture paths (`.dds`) against the requested descriptor path, so it can never fire. `00ad71c0` iterates two pointer vectors, not one, and its boolean is the `Foliage` game setting at `00f88a06`; `00af0b10` is the foliage group manager singleton `00f8c274` seeding its shader-time scalar to 1.0f. All ten packets of this map are now integrated.
