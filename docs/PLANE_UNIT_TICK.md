...
| 6 | A plane AI. | nothing in the ledger names one | Nothing yet. `007F3BA0 BSP_PlaneSquadron_TickAdvance` is reconstructed and also uncalled, and no `Plane*Ai`/`Plane*Think` name exists at all. Until steps 3-5 land there is nothing for an AI to command. |
