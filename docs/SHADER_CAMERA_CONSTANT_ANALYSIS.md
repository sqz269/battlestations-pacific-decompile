...
cache reconstruction subsequently joined the complete typed system prefix;
see `CAMERA_FRAME_STATE.md` and `SYSTEM_CONSTANT_BUILDER.md` for current scope.
Native camera construction and gameplay remain open. The subsequent
`CAMERA_MATRIX_DEPENDENCIES.md` clarifies that
