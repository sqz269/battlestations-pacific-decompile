...
Source commit `6860d0e11763044eda6b8216b510965d642bbc0b` contains the verified source. The build ran with the same owned source changes on checkout `6d622cff166837c24eb337a4d554e6517c32148f`; it is not described as a build of an already existing commit. The immutable local manifest retains 291 inputs and 140 artifacts, including all 18 linked production objects, compiler/header/library inputs, original reference bytes and Ghidra receipts. Prior AS/AT manifests are retained. The three stored-body call attribution failures remain explicit.
