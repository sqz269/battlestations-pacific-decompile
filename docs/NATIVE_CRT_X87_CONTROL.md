...
Primary registered the full source entries, passed the strict Win32 build, both existing CTests and eight fresh original seeds, and used the same frozen main library `aa4a9f060ade446e7638fed51f8977610df500a5c070984af473e5d8cc5eb3ae` for all three packets. All 37 sealed worker files, 36 report pins and two fresh spans passed. Both complete main archived bodies match all 36 original bytes and 11 instructions exactly, with zero relocations or providers across the five-section object. CALL and CW-top JMP contracts remain distinct and assembly-only; no runtime claim is added. Reviewed names and evidence comments are saved with prior values retained; correct CRT library names remain. All affected exports were forcibly refreshed. Immutable primary evidence: `local/x87_control_primary/`. New source interfaces are not original caller ABI or gameplay validation.
