...
Read-only BSP queries verify `C:/Users/sqz269/bsp.gpr`, `/battlestationspacific.exe`. Saved bytes match the installed PE for all four spans. A43180 and A430E0 currently have **no Ghidra function**. The full linear disk listings end at the RETs above and are followed by CC padding. All six direct call rows therefore remain real validation failures until the integrator defines those bodies under its write lock. No Ghidra mutation, annotation or save is performed by this packet.
