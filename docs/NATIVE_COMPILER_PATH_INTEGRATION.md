...
## Actual online client leaves

Code `a602d7cd3f6a728beda3645e8b807ba7f7d6cc71` supplies A43180/A430E0 and raw manager getters A3E510/A3EAC0. The current publication reloads, PUSH ECX output preimages, unsigned user gate and unnormalized virtual AL reach the required session-policy boundary. Strict Win32 build, two CTests and ten original/source cases pass. Two saved function definitions close the six worker call failures; all 22 call rows and four saved annotation readbacks pass. `local/checkpoints/a602d7cd/native-online-client-default/validation.json` freezes 3206 artifacts. The downstream771630 body, raw game/client construction, application composition and gameplay remain open. Earlier lifetime, pump and storage checkpoints retain their separate scope.

## Renderer registry raw lifetime binding

Code `038b2942b68e3041cfacf73a33ce36a55f2c419a` routes twelve existing state-definition/system-constant bodies through the actual01090AA0 manager and actual registry publication cells. It shares existing literal tables and array cleanup. Strict Win32 build, both CTests and the source fixture normal/failure modes pass. Root repaired B58320 call overrides and stored body, verified758 direct call rows and12 saved annotations. `local/checkpoints/038b2942/native-renderer-registry-default/validation.json` freezes 3240 artifacts. This migration adds no new normal bodies and makes no differential/gameplay claim. A failed system operation must still retain its owner/frame, preventing composition with the original renderer parent allocation cleanup. Base, Lua owner, control-worker and renderer container work remains separate.

## Actual renderer base lifetime

Code `8f73fbf754bc47dfac527e898c3cf7a4cc6ebfa4` reconstructs nine base/primary/singleton-subobject bodies. The same F8D394 primary and01090AA0 manager cells preserve the registered primary+0C identity. Strict Win32 build,2 CTests and the existing raw-owner source fixture pass. Root saved9 annotations and restored3 scalar stack adjustments; all24 direct call rows pass. `local/checkpoints/8f73fbf7/native-renderer-base-default/validation.json` freezes 3229 artifacts. The full B32410/B32920 derived lifetime, source EndFrame dependencies, canonical application drain and gameplay remain unresolved. B32920 saved body currently truncates at B32D72; later cleanup calls have native disk evidence only until its full tail is recovered.
