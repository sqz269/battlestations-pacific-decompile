# Native Tracer Update

Addresses: 00b74390, 00ba9960, 00baa510, 00baa670, 00baabb0. Descriptive names are hypotheses, not recovered symbols.
...
| --- | --- | --- |
| `00b74390` | `set_native_generated_model_bounds_00b74390` | ECX actual model, stack four-word pointer; RET4 |
| `00ba9960` | `set_native_tracer_alpha_00ba9960` | ECX actual7ACh tracer, stack alpha; RET4 |
| `00baa510` | `predicate_native_tracer_00baa510` | ECX actual7ACh tracer; AL result; RET |
| `00baa670` | `append_native_tracer_point_00baa670` | ECX actual7ACh tracer, stack position/direction pointers; RET8 |
| `00baabb0` | `update_native_tracer_00baabb0` | ECX actual7ACh tracer; six stack words matrix/age/position/direction/speed/subject58; RET18 |

`00b74390`: Four forward x87 stores to08/0C/10/14 followed by flags138 & FFFFFFCF | B.

`00ba9960`: Current material1BC diffuse38+0C raw MOVSS, then reload optional child254, actual geometry180/element0/material20 and its diffuse alpha. No dirty flag.

`00baa510`: Native201/200 short paths, local202 inactive7A8 comparison including unordered, or actual linked1B8 time10/currentage1C4/reciprocal1EC sqrt fade and current linked alpha24 threshold CE9BAC.

`00baa670`: Mutates actual30h ring194/index1AC/count198, cuts reused next backlink, publishes current1B8/indices/count, width184/188 and alpha spills, linked lengths and cumulative distance. No allocation or substitute ring.

`00baabb0`: Complete world/local update, normalized direction/projection, current34 capture/matrix dispatch, ring insertion and all-chain bounds, actual geometry description counts, fade-in, visibility and stop alpha. Matrix argument and final subject58 are marshalled but unread.
