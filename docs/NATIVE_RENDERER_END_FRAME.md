...
Queue execution and two overlay bodies remain required substantive providers.
...
- B2BB90 (1765 bytes) and B2B580 (1549 bytes). Their full resource/material/draw
  graphs remain external; their initial gates are not substituted for those bodies.
  Current renderer +C4 B28D00 is supplied by native_renderer_debug_lines with its
  explicit borrowed context and supported-domain requirements.
...
The three modules are reused as components with an explicit remaining-provider
interface. No production worker/application callback is bound. The source still
requires substantive raw B1EBE0 queue execution, B2BB90 debug-sphere records and
B2B580 sprite records. The semantic RenderCommandQueue interface does not supply
the raw queue contract. Separate later source for B2BB90 does not satisfy this
interface without its real model/material/provider composition.

The only production adaptation is in the debug-line module: its two shader
bind calls use main's current fastcall argument order, renderer, synchronization
pointer, logical-shader pointer. Shared shader/owner/model implementations are
unchanged. R34 freshly checks PE/live bytes and relinks retained component
fixtures against the strict Win32 build. Inactive EndFrame wrapper checks,
physical rewinds, synthetic-COM query polling, and real-D3D9 debug-line source
checks do not establish active full EndFrame, XLiveRender frame execution,
worker adoption or game rendering. Current evidence is in
`reports/native_renderer_end_frame_main_r34.json`; copied older reports retain
their historical revisions and paths.
