...
Primary verified all 30 sealed worker pins, 51 additional report pins, and freshly reread all 34 guarded spans (1,872 bytes). The physical pool allocation dependency has since been completed on main in b392be9b; its incomplete label above describes the pinned discovery base. The 834-byte startup parent still has unresolved gamma and reset dependencies. Immutable evidence: `local/resource_init_discovery_primary/`.

Current main also contains full raw B237D0 dynamic-buffer release and B23B10 resource restore (`native_dynamic_buffer_device_release.cpp`, `native_renderer_resource_restore.cpp`), confirmed by the current source and address ledger. The reset-parent dependency line above is a historical discovery summary: B262C0 resource release and B29670 device recreation still require reconstruction. Their transitive readiness needs a separate audit.
